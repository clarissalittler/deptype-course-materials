...
This stage aims for a **logically sound implementation** with stronger normalization (NBE) and universe discipline. It also documents why positivity checks are required for full consistency.
...
To keep the logic consistent, inductive definitions must be **strictly positive** in their recursive occurrences. Without a positivity check, it's possible to define types that encode paradoxes (e.g., Girard-like contradictions) and collapse the system. This stage documents the requirement and where a positivity checker would plug into the implementation.
...
For **dependent** matches, provide a motive with `return`:
...
`P` is the **motive** (the result type as a function of the scrutinee).
