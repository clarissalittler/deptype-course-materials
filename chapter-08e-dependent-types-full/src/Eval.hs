...
matchPattern = go
  where
    ...
    go (PCon "Zero" []) Zero = Just Map.empty
    go (PCon "Succ" [p]) (Succ t) = go p t
    go (PCon "True" []) TmTrue = Just Map.empty
    go (PCon "False" []) TmFalse = Just Map.empty
    go (PCon "TT" []) TT = Just Map.empty
    ...
    mergeAll :: [Map Name Term] -> Maybe (Map Name Term)
    mergeAll = foldl merge (Just Map.empty)

    merge :: Maybe (Map Name Term) -> Map Name Term -> Maybe (Map Name Term)
    merge Nothing _ = Nothing
    merge (Just acc) m =
      Map.foldlWithKey' mergeStep (Just acc) m
      where
        mergeStep Nothing _ _ = Nothing
        mergeStep (Just acc') k v =
          case Map.lookup k acc' of
            Nothing -> Just (Map.insert k v acc')
            Just v' ->
              if alphaEq v v'
                then Just acc'
                else Nothing
...
step = \case
  ...
  -- Pattern matching
  Match v _ branches | isValue v -> tryMatch v branches
  Match t motive branches | not (isValue t) ->
    (\t' -> Match t' motive branches) <$> step t
