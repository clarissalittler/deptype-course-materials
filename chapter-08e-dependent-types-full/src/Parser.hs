...
reserved = ["Type", "Nat", "Bool", "Unit", "Empty", "true", "false", "zero",
            ...
            "refl", "Eq", "match", "with", "return", "natElim", "boolElim",
            "unitElim", "emptyElim", "J"]
...
termP = makeExprParser termAtom termOps
  where
    ...
    -- match t return P with | pat -> rhs | ...
    matchP = do
      _ <- symbol "match"
      t <- termP
      motive <- optional (symbol "return" *> termP)
      _ <- symbol "with"
      branches <- many branchP
      return (Match t motive branches)
...
patternP =
  psuccP <|> pzeroP <|> ptrueP <|> pfalseP <|> punitP <|> pconP <|> pvarP <|> pwildP
  where
    pvarP = PVar <$> varP
    pwildP = symbol "_" >> return PWild
    pzeroP = symbol "zero" >> return (PCon "Zero" [])
    psuccP = do
      _ <- symbol "succ"
      p <- patternAtom
      return (PCon "Succ" [p])
    ptrueP = symbol "true" >> return (PCon "True" [])
    pfalseP = symbol "false" >> return (PCon "False" [])
    punitP = symbol "TT" >> return (PCon "TT" [])
    pconP = try $ do
      name <- upperIdentifier
    ...
    patternAtom =
      parens patternP <|> psuccP <|> pzeroP <|> ptrueP <|> pfalseP <|> punitP <|> pconP <|> pvarP <|> pwildP
