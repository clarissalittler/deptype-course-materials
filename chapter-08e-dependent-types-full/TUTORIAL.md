...
```
append A m n xs ys = match xs return (λ_. Vec (m + n) A) with
...
Implementation:
```
lookup A n xs i = match xs return (λ_. A) with
...
```
head : Vec (succ n) A → A
head v = match v return (λ_. A) with
  | Cons _ _ x xs -> x
...
```
tail : Π(A:Type). Π(n:Nat). Vec (succ n) A → Vec n A
tail A n v = match v return (λ_. Vec n A) with
  | Cons _ k x xs -> xs
```
