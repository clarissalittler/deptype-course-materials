...
import Syntax
import Constraint (Constraint(..), ConstraintSet, Subst)
...
prettyConstraints cs = unlines $ map (\c -> "  " ++ prettyConstraint c) cs

-- | Pretty-print a substitution
prettySubst :: Subst -> String
prettySubst s
  | Map.null s = "{}"
  | otherwise = "{\n" ++ unlines (map showBinding (Map.toList s)) ++ "}"
  where
    showBinding (var, ty) = "  " ++ var ++ " ↦ " ++ prettyType ty
